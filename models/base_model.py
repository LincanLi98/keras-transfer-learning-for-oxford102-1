...
class BaseModel(object):
    ...
    def _create(self):
        raise NotImplementedError('subclasses must override _create()')#report errors, ususlly covered by his son
    ...
    def load(self):
        ...
        self.model.load_weights(config.get_fine_tuned_weights_path())#model load weitht from "G:\keras-transfer-learning-for-oxford102\trained\fine-tuned-resnet50-weights.h5"
        return self.model

    @staticmethod
    def save_classes():
        joblib.dump(config.classes, config.get_classes_path())#path to classes-resnet50: "G:\keras-transfer-learning-for-oxford102\trained\classes-resnet50"
    ...
    @staticmethod
    def load_classes():
        config.classes = joblib.load(config.get_classes_path())#return the path to classes-resnet50 (G:\keras-transfer-learning-for-oxford102\trained)

    def load_img(self, img_path):
        img = image.load_img(img_path, target_size=self.img_size)#load image
        x = image.img_to_array(img)#transform the format of pictures to array
        x = np.expand_dims(x, axis=0)#expand dim at axis=0
